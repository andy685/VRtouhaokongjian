  ...
  { title: '创建时间', key: 'createTime', width: 160 },
  {
    ...
    render(row) {
      return h(NSpace, { size: 4 }, { default: () => [
        h(NButton, { type: 'primary', size: 'small', text: true, onClick: () => openDetail(row) }, { default: () => '详情' }),
        isMarked(row.orderNo)
          ? h(NButton, { type: 'warning', size: 'small', text: true, onClick: () => { unmarkOrder(row.orderNo); window.$message?.info('已取消标记') } }, { default: () => '取消标记' })
          : h(NButton, { type: 'warning', size: 'small', text: true, onClick: () => { if (!openMarkDialog({ orderNo: row.orderNo, store: shopOptions.find(o => o.value === row.shop)?.label || row.shop, amount: row.giftValue })) window.$message?.warning('该订单已在异常列表中') } }, { default: () => '标记异常' }),
      ] })
    }
  },
]
