  ...
  <div class="diagnostic-page">
            ...
            <article v-for="report in group.items" :key="report.id" class="report-card" :class="healthInfo(report.score).className" @click="openReport(report.month)">
              <div class="report-card-main">
                <div class="report-copy">
                  <div class="report-card-top"><span>{{ report.listPeriod }}</span><n-tag size="small" round :bordered="false" class="period-tag">{{ report.periodType }}</n-tag><span v-if="reportUpdateStates[report.id]" class="updating-badge">报告更新中</span><span v-else-if="report.isNew" class="new-badge">新报告</span></div>
    ...
    <template v-if="activeReport">
      ...
      <section class="report-hero" :style="heroStyle">
        ...
        <div class="hero-heading"><h1>{{ activeReport.store }}</h1><n-tag size="small" round class="hero-period-tag">{{ activeReport.periodType }}</n-tag></div>
        <p>基于已固化的经营数据快照、诊断资料与报告模板自动生成。</p>
        <div class="hero-meta"><div><span>所属商家</span><strong>{{ currentStore.merchant }}</strong></div><div><span>数据范围</span><strong>{{ activeReport.periodRange }}</strong></div><div><span>生成时间</span><strong>{{ activeReport.generatedAt }}</strong></div></div>
      </section>
      ...
      <section class="report-section game-diagnosis"><div class="section-title"><div><h2>内容与游戏诊断</h2><p>以点播、完成和收入贡献识别内容机会与风险</p></div></div><div class="game-grid"><article><span>核心贡献</span><h3>《星际营救》《奇幻赛车》</h3><p>合计贡献 46% 点播收入，完成率稳定在 84% 以上，适合纳入亲子套餐与导购推荐。</p></article><article><span>重点关注</span><h3>《深海探险》</h3><p>启动率较高但完成率仅 58%，建议优先检查设备适配和新手引导。</p></article><article><span>内容结构</span><h3>热门内容集中度偏高</h3><p>TOP5 游戏收入占比 61%，尾部游戏占比 18%，建议调整推荐位并降低低效内容资源投入。</p></article></div></section>
      ...
      <section class="report-section insights-section"><div class="section-title"><div><h2>问题、机会与建议</h2><p>以下结论均基于本期数据快照</p></div></div><div class="insight-grid"><article class="insight problem"><span>01</span><h3>主要问题</h3><p>{{ activeReport.problems.join('；') }}</p></article><article class="insight chance"><span>02</span><h3>增长机会</h3><p>{{ activeReport.opportunities.join('；') }}</p></article><article class="insight action"><span>03</span><h3>优先动作</h3><p>{{ activeReport.actions.filter((item) => item.priority === 'P0').map((item) => item.action).join('；') }}</p></article></div></section>
...
.period-tag{--n-color:#e0e7ff!important;--n-text-color:#3730a3!important;flex-shrink:0;font-weight:600}
...
.hero-heading{display:flex;align-items:center;gap:12px;position:relative;z-index:1;margin:20px 0 10px}.hero-heading h1{margin:0}.hero-period-tag{--n-color:rgba(255,255,255,.14)!important;--n-text-color:#fff!important;--n-border:1px solid rgba(255,255,255,.34)!important;flex-shrink:0;font-weight:600}
...
@media(max-width:760px){.report-hero{padding:24px}.report-hero h1{font-size:27px}.hero-heading{align-items:flex-start;flex-direction:column;gap:8px}.report-hero .hero-meta{grid-template-columns:1fr;gap:8px}}
...
.report-content .insights-section{padding:26px}.report-content .insight-grid{display:grid;grid-template-columns:repeat(3,1fr);gap:16px;padding:0;border:0;background:transparent}
...
.report-content .insight.problem{background:#fff7ed;border-color:#fed7aa}.report-content .insight.chance{background:#f0fdf4;border-color:#bbf7d0}.report-content .insight.action{background:#fff;border-color:#dbe4f0}
