<template>
  <div class="page-container animate-fade-in">
    <div class="page-header">
      <div>
        <h1>收款账户</h1>
        <p class="header-desc">管理银行账户信息，用于接收游戏收益结算款项（平台按期自动打款）</p>
      </div>
    </div>
    ...
    <n-alert type="info" :bordered="false" style="margin-bottom: 24px;">
      ...
      平台将按照结算周期自动生成结算单并打款到此账户，无需手动申请提现。
    ...
    <div class="bank-section">
      <div class="section-header">
        <h3>银行账户</h3>
        <n-button v-if="!isEditing && !pendingChange" text type="primary" @click="startEdit">
      ...
      <n-empty v-else-if="!bankAccount && !pendingChange" description="未绑定银行账户，点击编辑按钮添加" />
    </div>
...
// 银行账户信息
const bankAccount = ref<{ bankName: string; bankNameText: string; cardNo: string; holderName: string } | null>({
  bankName: 'CMB',
  bankNameText: '招商银行',
...
const bankForm = ref({ bankName: '', cardNo: '', holderName: '' })
...
function startEdit() {
  if (bankAccount.value) {
    const { bankNameText, ...rest } = bankAccount.value
    bankForm.value = { ...rest }
  } else {
    bankForm.value = { bankName: '', cardNo: '', holderName: '' }
  }
  isEditing.value = true
}

async function saveBankInfo() {
  if (!bankForm.value.bankName || !bankForm.value.cardNo || !bankForm.value.holderName) {
...
async function confirmVerifyCode() {
  ...
  setTimeout(() => {
    ...
    if (verifyCode.value === '123456') {
      ...
      addOperationLog('warning', '账户修改申请已提交', `新账户：${bankNameText} - ${formatCardNo(bankForm.value.cardNo)}，进入10分钟冷却期`)
...
function handleCoolingFinish() {
  if (pendingChange.value) {
    bankAccount.value = {
      ...
      holderName: pendingChange.value.holderName,
    }
...
function loadOperationLogs() {
  ...
  if (operationLogs.value.length === 0 && bankAccount.value) {
    addOperationLog('success', '银行账户已绑定', `${bankAccount.value.bankNameText} - ${formatCardNo(bankAccount.value.cardNo)}`)
  }
}
