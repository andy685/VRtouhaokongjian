  ...
  <div class="page-container animate-fade-in">
    ...
    <div class="page-header">
      <div>
        <h1>分账管理</h1>
        <p class="header-desc">
          结算单生成后<strong class="highlight">正常单自动提交拉卡拉分账</strong>，
          仅异常单（账户缺失/金额为0/有争议等）需人工处理。
    ...
    <n-alert type="success" :bordered="false" style="margin-bottom: 20px; border-radius: 12px;">
      ...
      符合以下条件的结算单将<strong>自动提交拉卡拉分账接口</strong>：
      收款账户完整 且 应发分润 ≥ ¥100 且 无申诉/冻结标记。
      不满足任一条件则进入「异常待处理」队列。
    </n-alert>
    ...
    <div class="content-card">
      <div class="table-toolbar">
        ...
        <n-space align="center">
          ...
          <n-select v-model:value="filterAgent" placeholder="全部代理商" :options="agentOptions" clearable size="small" style="width: 150px;" />
    ...
    <n-modal v-model:show="showResolveModal" preset="dialog" type="warning" title="异常处理 - 放行分账">
      ...
      <n-descriptions label-placement="left" :column=1 bordered size="small" style="margin-top:12px;">
        <n-descriptions-item label="结算单号">{{ resolveTarget?.settlementNo }}</n-descriptions-item>
        <n-descriptions-item label="代理商">{{ resolveTarget?.agentName }}</n-descriptions-item>
        <n-descriptions-item label="原异常原因">{{ resolveTarget?.exceptionReason }}</n-descriptions-item>
        <n-descriptions-item label="应发分润">
          <strong style="color:#4F46E5;">¥{{ resolveTarget?.commissionAmount?.toLocaleString() }}</strong>
        </n-descriptions-item>
      </n-descriptions>
      <n-input v-model:value="resolveNote" type="textarea" placeholder="请输入处理说明（如：已联系代理商补全账户信息，现可正常打款）..." :autosize="{ minRows: 2 }" style="margin-top: 12px;" />
      <template #action>
        <n-space justify="end">
...
const sumBy = (status: SettlementStatus, field: 'commissionAmount') =>
  payoutData.value.filter(d => d.status === status).reduce((s, d) => s + d[field], 0)

const autoSubmittedAmount = computed(() => sumBy('auto_submitted', 'commissionAmount'))
const processingAmount = computed(() => sumBy('processing', 'commissionAmount'))
const settledAmount = computed(() => sumBy('settled', 'commissionAmount'))
...
const batchSelectedTotal = computed(() =>
  Array.from(selectedExceptionKeys.value).reduce((sum, id) => {
    const r = payoutData.value.find(d => d.id === id)
...
const resolveTarget = ref<SettlementRecord | null>(null)
const resolveNote = ref('')
function handleResolve(record: SettlementRecord) {
...
function confirmResolve() {
  ...
  r.remark = resolveNote.value || '人工放行'
  r.logs.push({
    action: '人工放行', time: new Date().toLocaleString('zh-CN'),
    detail: resolveNote.value || '管理员人工审核后放行，将重新提交拉卡拉', type: 'success' as const
  })
  message.success(`${r.settlementNo} 已放行，将自动提交拉卡拉分账`)
  showResolveModal.value = false
  resolveNote.value = ''
}

// ========== 异常处理：重试（失败单修复后）==========
function handleRetry(record: SettlementRecord) {
  ...
  message.success(`${record.settlementNo} 已重新提交`)
  showDetailModal.value = false
}

// ========== 单笔查询拉卡拉状态 ==========
function handleQueryOne(record: SettlementRecord) {
...
function confirmBatchResolve() {
  ...
  payoutData.value.forEach(d => {
    if (selectedExceptionKeys.value.has(d.id) && (d.status === 'exception' || d.status === 'failed')) {
      ...
      d.logs.push({
        action: '批量放行', time: new Date().toLocaleString('zh-CN'),
        detail: '管理员批量人工放行', type: 'success' as const
      })
      count++
    }
  })
