...
const hostMonitorColumns = [
  ...
  { title: () => columnTip('复玩率', '在该设备完成体验后，再次回到门店或再次下单体验的用户占比，用于衡量内容与设备的复购吸引力。'), key: 'replayRate', width: 80, align:'center' as const, render: (row: any) => `${row.replayRate}%` },
  { title: () => columnTip('带动办卡', '该设备在体验过程中带动玩家注册会员或办理会员卡的人数，用于衡量设备对会员转化的贡献。'), key: 'cardLeadCount', width: 90, align:'center' as const, render: (row: any) => `${row.cardLeadCount}人` },
