...
> 计算口径边界：健康分技术实现可按 `STORE_HEALTH_V1` 更新，但本原型不要求调整报告前端展示模块。核心经营指标、内容与游戏诊断、游戏点播数据继续保持当前结构；“TOP5 游戏营收占比”保留在游戏点播数据中。公式中的内容覆盖率、时长达成率、内容复玩率可作为 AI 解读依据，不直接替换前端 KPI。
