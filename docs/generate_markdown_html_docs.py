...
DOCS = [
    ...
    },
    {
        ...
        "brand": "功能规格说明书",
        "subtitle": "页面功能、交互状态、业务规则与 API 字段",
    },
    {
        "md": ROOT / "用户协议.md",
        "html": ROOT / "用户协议.html",
...
def make_toc(md_text: str):
    ...
    for line in md_text.splitlines():
        ...
        if not m:
            ...
            continue
        hashes, raw = m.groups()
        plain = re.sub(r"\s+\{#.*\}\s*$", "", raw.strip())
        anchor = slugify(re.sub(r"[`*_]", "", plain))
...
def build_html(body: str, toc: list[tuple[int, str, str]], title: str, brand: str, subtitle: str) -> str:
    ...
    * { box-sizing:border-box; }
    ...
    a { color:inherit; }
    .sidebar {
      position:fixed; inset:0 auto 0 0; width:280px; padding:20px 0;
      background:#0a0f1a; border-right:1px solid var(--border); overflow-y:auto;
    }
    .logo { padding:0 20px 16px; border-bottom:1px solid var(--border); margin-bottom:12px; }
    .logo h2 {
      margin:0; font-size:18px; background:linear-gradient(135deg,var(--accent),var(--accent2));
      -webkit-background-clip:text; -webkit-text-fill-color:transparent;
    }
    .logo .ver { margin-top:4px; font-size:12px; color:var(--muted); }
    .logo .sub { margin-top:8px; font-size:12px; color:#cbd5e1; line-height:1.6; }
    .sidebar a {
      display:block; padding:7px 20px; text-decoration:none; color:var(--muted);
    ...
    .main { margin-left:280px; padding:40px 56px 96px; max-width:1080px; }
    .hero { margin-bottom:28px; padding-bottom:18px; border-bottom:1px solid var(--border); }
    .hero h1 {
      margin:0 0 8px; font-size:30px;
      background:linear-gradient(135deg,var(--accent),var(--accent2));
      -webkit-background-clip:text; -webkit-text-fill-color:transparent;
    }
    .hero .meta { font-size:13px; line-height:1.8; color:var(--muted); }
    .hero .pill {
      display:inline-block; margin-top:12px; padding:6px 10px; border-radius:999px;
      background:rgba(99,102,241,.12); color:#c4b5fd; font-size:12px; border:1px solid rgba(99,102,241,.28);
    }
    h1,h2,h3,h4 { scroll-margin-top:24px; }
    h2 { margin:40px 0 18px; padding-bottom:8px; border-bottom:2px solid var(--accent); font-size:22px; }
    h3 { margin:28px 0 12px; color:#c4b5fd; font-size:17px; }
    h4 { margin:20px 0 10px; color:#93c5fd; font-size:15px; }
    p,li { line-height:1.85; font-size:14px; color:var(--text); }
    ul,ol { padding-left:24px; }
    table { width:100%; border-collapse:collapse; margin:14px 0 24px; font-size:13px; }
    th { background:#1a1a3e; color:#a78bfa; border:1px solid var(--border); text-align:left; padding:8px 10px; }
    td { border:1px solid var(--border); padding:7px 10px; color:var(--text); vertical-align:top; }
    tr:hover td { background:rgba(99,102,241,.04); }
    code { font-family:"SF Mono","JetBrains Mono",Consolas,monospace; font-size:12px; color:var(--blue); }
    pre {
      background:#0a0f1a; border:1px solid var(--border); border-radius:10px; padding:16px;
      overflow-x:auto; margin:14px 0;
    }
    pre code { color:var(--text); font-size:12px; line-height:1.65; white-space:pre; }
    ...
    .footer { margin-top:54px; padding-top:18px; border-top:1px solid var(--border); color:var(--muted); font-size:12px; }
    @media (max-width:960px) {
      .sidebar { position:static; width:auto; height:auto; border-right:0; border-bottom:1px solid var(--border); }
      .main { margin-left:0; padding:24px 20px 72px; }
    }
    """
...
def main() -> int:
    for doc in DOCS:
        ...
        marked_md, toc = make_toc(md_text)
        body = render_markdown(marked_md)
        doc["html"].write_text(
            build_html(body, toc, doc["title"], doc["brand"], doc["subtitle"]),
            encoding="utf-8",
        )
