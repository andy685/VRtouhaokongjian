...
**⑤ 健康分计算**：统一截断函数 `C(x) = min(100, max(0, x))`；健康分 = 营收增长力×30% + 客流活跃度×18% + 会员质量×24% + 内容吸引力×12% + 运营执行力×16%。等级：80–100 健康增长、70–79 稳中待升、0–69 重点优化。

**⑥ 五维计算方式**：

| 维度 | 计算公式 |
|------|---------|
| 营收增长力 | 营收环比分×50% + 客单价环比分×20% + 同类门店营收表现分×30%；环比分 = C(50 + 2.5×环比百分点)；同类门店表现分按本店营收相对同城同店型中位数线性插值（+20%→100 分、0%→50 分、−20%→0 分）。门店自设目标不参与评分，目标完成率（实收÷月度目标）仅在报告中展示 |
| 客流活跃度 | 日均订单分×40% + 高峰转化分×35% + 非高峰转化分×25%；日均订单分 = C(50 + 2.5×((本期日均订单÷基准日均订单−1)×100))，基准优先取本店近 3 个完整周期；高峰/非高峰转化分 = C((转化率−35%/20%)÷30%×100)；高峰时段由门店配置（如周末 14:00–21:00、工作日 18:00–21:00） |
| 会员质量 | 90 天复购分×45% + 储值转化分×35% + 沉睡会员分×20%；90 天复购分 = C((90天复购率−10%)÷25%×100)；储值转化分 = C((储值转化率−3%)÷15%×100)；沉睡会员分 = C((25%−沉睡会员占比)÷20%×100) |
| 内容吸引力 | TOP5 集中度健康分×40% + 尾部内容健康分×30% + 内容完成率分×30%；TOP5 游戏营收占比 = TOP5 游戏实收营收 ÷ 全部游戏实收营收；45%–65% 为理想区间，低于 25% 视为缺少核心爆款，高于 80% 视为过度依赖头部内容；尾部健康分 = C((35%−尾部内容占比)÷25%×100)；完成率分 = C((内容完成率−50%)÷30%×100)（完成率=正常完成会话数÷启动会话数） |
| 运营执行力 | 行动完成分×40% + 活动转化分×35% + 数据完整分×25%；行动完成分 = C(100×已完成行动数÷应完成行动数)；活动转化分 = C((活动转化率−3%)÷9%×100)（活动转化率=活动带来的有效支付订单数÷触达或参与人数）；数据完整分 = C((数据完整率−85%)÷15%×100)（数据完整率=已具备必要字段记录数÷应具备记录数） |
...
**入口**：总运营后台 `AI店招机器人`（设备管理 / 知识库管理 / 声音复刻 / 背景管理 / API 密钥配置）；商家后台 `AI店招机器人 > 设备管理`
...
| 门店知识（Store） | 「关键词 + 回答」条目（启用开关） | 店铺日常维护，平台可编辑 | 命中关键词优先回答，未命中走向量检索；门店不接触百炼控制台 |
| 活动知识（Campaign） | 「关键词 + 回答」条目（启用开关） | 店铺维护 | 时效性活动口径，命中优先；提醒避免夸大宣传 |
