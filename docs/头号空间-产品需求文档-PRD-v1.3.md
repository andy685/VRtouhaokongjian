...
> **版本**: v2.3 统一版
> **日期**: 2026年6月16日
> **状态**: 优化版（代理商分润公式调整为扣除供应商成本；阶梯分润策略暂隐藏；预设销售金额CP端必填；全端游戏封面3:4竖屏统一）
> **密级**: 内部公开
> **作者**: 产品团队
...
**头号空间**是一套面向 **VR线下体验店** 的全链路运营管理系统。平台系统内第一版收费项为 **商家游戏豆采购费** 和 **提现/打款手续费**，代理商按辖区商家游戏豆采购额分润；硬件销售如另有商务安排，按线下合同处理，不纳入本系统结算。系统由 **四层架构** 组成：
...
| **代理商** | 区域分润、管理工具、结算透明、扣除供应商成本后按比例分润 |
...
平台系统内第一版收入来源为 **商家游戏豆采购费 + 提现/打款手续费**。代理商第一版收入只来自辖区商家游戏豆采购分润，不按硬件销售、商家交易流水或 C端消费订单分润。
...
**核心公式**：代理商分润 =（辖区商家月游戏豆采购额 − 供应商成本）× 分润比例

> **供应商成本**：平台从供应商（游戏CP）采购游戏豆所产生的成本。在计算代理商分润时，须先扣除供应商成本，再按比例计算分润，确保分润基于平台实际利润而非单纯采购流水。

**计算示例（以城市代理 5% 比例为例）**：

| 月辖区游戏豆采购额 | 供应商成本 | 分润基数 | 分润比例 | 应发分润 |
|:-------------------:|:----------:|:--------:|:--------:|:--------:|
| ¥120,000 | ¥66,000 | ¥54,000 | 5% | ¥2,700 |
| ¥250,000 | ¥137,500 | ¥112,500 | 5% | ¥5,625 |

> ⚠️ **变更说明（v2.2）**：阶梯分润策略（×0.8起步/×1.0基准/×1.2成长/×1.5奖励档位）**暂时隐藏，暂不启用**。当前仅按「扣除供应商成本后的净额 × 分润比例」计算，阶梯系数统一视为 ×1.0。

#### 代理商月度总分润

```
平台系统内代理商月总分润 = 游戏豆采购分润（硬件分润如有，按线下合同执行，不走平台系统）
  └── 游戏豆分润: ∑[(辖区商家月游戏豆采购额 − 供应商成本) × 分润比例]
```
...
| 🏬 **店铺概览** | 旗下店铺列表、状态筛选 |
| 💰 **分润明细** | 按月度查看分润计算过程（采购额 − 供应商成本 = 分润基数 × 比例）、明细表格、Excel导出 |
...
| 🤝 **代理商结算** | 全平台代理商分润概览、按级别筛选、扣除供应商成本后按比例计算、拉卡拉打款状态 | 超管/财务 |
| 🎮 **CP结算** | 有效体验次数、单次成本价、退款冲减、拉卡拉打款状态 | 超管/财务 |
| 📋 **阶梯策略配置** | ~~三级代理阶梯系数CRUD、版本管理~~ **（v2.2 暂隐藏，暂不启用）** | 超管 |
| 💰 **打款管理** | 4状态管理（待打款/打款中/已打款/失败）、失败提示、API同步、其他支付记录 | 财务 |
...
| 代理商 | 第一版仅按辖区商家游戏豆采购额计算 | 周期结算，通过拉卡拉分账/代付 |
| CP | 第一版按有效体验次数 × 单次成本价计算 | 周期结算，通过拉卡拉分账/代付 |
...
| 🏪 **商家结算** | 商家应结金额、充值待结、现金留存、拉卡拉分账状态、待核销金额 | 超管/财务 |
| 🤝 **代理商结算** | 全平台代理商分润概览、供应商成本扣除后按比例计算、拉卡拉打款状态 | 超管/财务 |
| 🎮 **CP结算** | 有效体验次数、单次成本价、退款冲减、拉卡拉打款状态 | 超管/财务 |
| 📋 **阶梯策略配置** | ~~城市/区域/省级三级阶梯系数的CRUD~~ **（v2.2 暂隐藏，暂不启用）** | 超管 |
| 💰 **打款管理** | 待打款/打款中/已打款/失败4状态管理+API同步+失败提示+其他支付记录 | 财务 |
| 🔍 **对账中心** | 平台×拉卡拉×商家×代理商×CP 五方比对+差异高亮+人工调账+报告导出(Excel/PDF) | 财务 |
| 📈 **财务报表** | 各类统计报表 | 超管/财务 |

#### 4.8.6 代理商结算页

> 这是**平台方查看和管理所有代理商分润数据的主页面**，是 v1.3 新增的核心页面。v2.2 更新公式为「扣除供应商成本后按比例分润」。
...
│ │ │   128    │ │ ¥490,675 │ │ ¥415,800 │ │ ¥74,875  │          │    │
│ │ └──────────┘ └──────────┘ └──────────┘ └──────────┘          │    │
│ └─────────────────────────────────────────────────────────────────┘    │
│                                                                      │
│ ┌─ 代理商分润明细表 ──────────────────────────────────────────────┐   │
│ │ ☑ │ 代理商        │ 级别  │ 月豆采购额 │供应商成本│分润基数│比例│应发分润  │
│ ├───┼───────────────┼──────┼──────────┼─────────┼───────┼────┼─────────┤
│ │ ☑ │ 广东省级总代-A │ 省级 │¥1,850,000│¥1,017,500│¥832,500│10% │¥83,250  │
│ │ ☑ │ 华东区域代理-B │ 区域 │¥ 680,000 │¥ 374,000│¥306,000│ 7% │¥21,420  │
│ │ ☑ │ 深圳城市代理-C │ 城市 │¥ 120,000 │¥ 66,000│¥54,000 │ 5% │¥2,700   │
│ │ ☑ │ 广州城市代理-D │ 城市 │¥ 45,000  │¥ 24,750│¥20,250 │ 5% │¥1,013   │
...
- 点击行可弹出该代理商的成本扣除计算明细（如上图的详情弹窗）
- 支持**批量操作**：勾选多个代理商 → 批量生成结算单 → 推送到打款队列
- **异常标红**：流水为0的代理商灰显；分润金额<¥100的提示"累积下月"；有争议/申诉的显示橙色警示

#### 4.8.7 阶梯策略配置页

> ⚠️ **v2.2 变更**：阶梯分润策略（×0.8起步/×1.0基准/×1.2成长/×1.5奖励档位）**暂时隐藏，暂不启用**。该页面路由已注释，菜单项已隐藏。当前分润统一按「扣除供应商成本后 × 分润比例」计算，阶梯系数视为 ×1.0。
...
结算引擎生成结算单
    ...
    │       │                   │
    │       │              回调成功 → 已到账(settled)
    │       │              回调失败 → 分账失败(failed) → 展示错误信息，运营判断重试或改用其他支付
    │       │
    │       └── 任一不通过 ──▶ 异常拦截(exception) → 展示原因，暂不自动提交
    │
...
| 分账失败 | `failed` | 拉卡拉返回错误（账户名不符/未开户等），展示错误并等待运营判断 | 失败 | 失败 | **是** |
| 异常拦截 | `exception` | 自动校验不通过、申诉冻结或资料缺失，暂不提交拉卡拉 | 异常 | 处理中 | **是** |
...
pending → reviewing → submitted → processing → settled (正常完成)
                        ↘ failed → [同步/重试/改其他支付] → submitted 或 closed
                        ↘ exception(拦截) → [补齐资料/解除冻结/重新校验] → submitted
...
| 代理申诉中 | `exception` | 代理对分润有异议发起申诉 → 暂冻结（复核后重新校验提交） |
...
| 区块 | 标题 | 内容 |
|------|------|------|
| 一 | 平台营收 | 收入漏斗：游戏豆销售 − 供应商成本 − 代理商分润 = 平台毛利 + 月度趋势折线图 |
...
商家月内多次采购游戏豆，月末统一汇总
    ...
    ▼
┌─────────────────────────────────────────────────────────┐
│  游戏豆分润引擎（v2.2：扣除供应商成本后按比例分润）         │
│                                                         │
│  商家A 本月游戏豆采购总额: ¥120,000                        │
│  城市代理-C 的分润计算:                                    │
│  ├─ 查代理商级别→城市代理→分润比例5%                      │
│  ├─ 查供应商成本: ¥66,000                                 │
│  ├─ 分润基数 = ¥120,000 − ¥66,000 = ¥54,000              │
│  └─ 分润 = ¥54,000 × 5% = ¥2,700                        │
│                                                         │
│  分润计算核心逻辑:                                        │
│  查代理商ID → 取级别 → 取分润比例                         │
│  → 累计当月商家游戏豆采购额 → 扣除供应商成本               │
│  → (采购额 − 供应商成本) × 分润比例                       │
│                                                         │
...
| 时间节点 | 操作 | 执行者 | 系统 |
|---------|------|--------|------|
| **游戏豆: 月末23:59** | 冻结当月数据，锁定各代理商辖区**游戏豆采购总额**和**供应商成本** | 定时任务(Cron) | 后端 |
| **次月1-3日** | 自动汇总各代理商的**（采购额 − 供应商成本）× 分润比例**，生成当月`SETTLEMENT`结算单草稿 | 结算引擎 | 后端 |
| **次月1-3日（生成后即时）** | 结算单自动校验 → 符合条件则**自动提交拉卡拉开票+分账接口**；不满足条件则进入异常队列并展示错误原因 | 系统(自动) | 分账引擎 |
| **次月4-7日** | 公示期（代理商可在自己的后台看到待发金额，有异议可发起申诉） | — | 双端可见 |
| **持续** | 拉卡拉异步回调更新状态：成功→已到账 / 失败→failed队列 | 系统(自动) | 拉卡拉Webhook |
| **随时** | 财务处置异常(exception/failed)、手动同步、重新提交或关闭 | 财务人员 | 分账管理 |
...
| **代理商账户修改中(冷却期)** | 正常生成分润，但结算单进入exception队列，等新账户生效后重新校验提交 |
| **争议/申诉** | 代理商发起申诉后对应结算单自动冻结为exception，财务复核后调整或重新提交 |
| **代理商合约终止** | 立即触发最终结算，清零所有未结游戏豆分润款项 |
| **拉卡拉分账失败** | 结算单变为failed状态，展示错误信息；运营判断重新同步、重试，或改为现金/其他形式支付 |

#### 5.3.4 两系统协作关系图

| 步骤 | 操作 | 时间节点 | 执行方 |
|:----:|------|:--------:|:------:|
| ① | 商家采购游戏豆 → 系统记录采购额及供应商成本 | 随时 | 商家 |
| ② | 扣除供应商成本后按比例分润计算 | 月末结算 | 系统自动 |
| ③ | 生成月度结算单初稿 | 次月1-3日 | 结算引擎 |
| ④ | 代理商确认：无异议→确认 / 有异议→申诉 | 次月4-7日 | 代理商 |
| ⑤ | 平台财务审核：正常→提交 / 申诉→复核 | 随时 | 财务人员 |
...
- 代理商分润第一版仅按（辖区商家游戏豆采购额 − 供应商成本）× 分润比例计算。
...
| **游戏豆订单** | 商家从平台的游戏豆采购记录，含供应商成本 | 商家后台 |
| **代理商结算单** | 每月代理商的分润结算记录（采购额 − 供应商成本 = 分润基数 × 比例） | 代理商系统/运营后台 |
| *（无系统级排队功能，由店员线下引导）* | — | — |
...
- 商家管理：辖区商家列表、经营数据查看
- 分润明细：按月查看分润计算过程（采购额 − 供应商成本 = 分润基数 × 比例）、Excel导出
...
| 代理商分润 | 第一版只实现游戏豆采购分润（扣除供应商成本后按比例计算），不实现商家交易流水或平台服务费分润；阶梯系数暂隐藏 |
...
| CP | 形成应结 8.00（即供应商成本） |
| 代理商 | 不按本单 C端消费分润；后续按辖区（游戏豆采购额 − 供应商成本）× 分润比例月结 |
...
```
代理商月分润 = (辖区商家月游戏豆采购额 − 供应商成本) × 分润比例
```

> **供应商成本**：平台从游戏CP采购游戏豆所产生的成本，在计算代理商分润时须先扣除。
...
| 节点 | 规则 |
|------|------|
| 月末 23:59 | 冻结当月游戏豆采购额和供应商成本 |
| 次月 1-3 日 | 生成代理商结算单 |
| 次月 4-7 日 | 公示和申诉 |
| 次月 15 日前 | 完成拉卡拉打款或异常处理 |
| 低于 100 元 | 不生成打款请求，累积到下月 |

如果游戏豆采购订单退款或撤销，按采购流水冲减代理商分润和供应商成本。C端消费订单退款不直接冲减代理商分润，除非同时触发游戏豆采购退款。
...
CP应结金额来自商家购买/消耗游戏豆形成的 B 端收入池，不来自玩家消费订单实收金额。第一版不实现“按玩家消费收入比例分成”。如后续 CP 合同改为按游戏豆采购额比例分成，必须新增独立版本、字段和计算公式，不得复用 C 端消费分成字段。
...
📎 v2.3 更新: 预设销售金额设为CP端必填项（§4.9.3，min=0.01）；全端游戏封面统一为 3:4 竖屏展示比例（收银台72×96 / 游戏库3:4 / 内容分发36×48）；会员扣费页筛选下拉框边框色与搜索框统一（CSS变量驱动）
