...
| 游戏点播数据 | 点播总量汇总指标（点播总次数、点播总时长、游戏营收占比、人均游玩时长，含环比）+ 热门游戏 TOP 榜单（游戏名称、点播次数、点播时长、营收占比、环比、表现标签：明星/稳定/待优化），支撑内容运营与游戏采购建议 |
...
| 内容吸引力 | 热门内容转化分×40% + 尾部内容健康分×30% + 内容完成率分×30%；热门转化分 = C((TOP 游戏订单占比−30%)÷30%×100)；尾部健康分 = C((35%−尾部内容订单占比)÷25%×100)（尾部=订单占比低于 4% 的游戏）；完成率分 = C((内容完成率−50%)÷30%×100)（完成率=正常完成会话数÷启动会话数） |
