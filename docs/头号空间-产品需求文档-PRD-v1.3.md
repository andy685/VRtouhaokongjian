...
> **版本**: v2.28
> **日期**: 2026年9月15日
> **状态**: 更新 AI 店铺经营诊断页面结构与交互规范（§4.6.4）
> **密级**: 内部公开
> **作者**: 产品团队
...
| 报告详情（健康分区） | 经营健康分卡（按段位联动配色）+ AI 六维诊断进度条（按 80/70 阈值分段着色） |
...
| 重点优化 | < 70 | 红色（`#EF4444` 系） | 同上；六维进度条中 < 70 的维度单独标红 |
...
| 配置区 | 可配置内容 | 校验规则 |
|--------|-----------|---------|
| 诊断维度与权重 | AI 诊断维度名称、每维权重（%）、每维数据来源说明；健康分计算方式（六维简单平均 / 按权重加权）。维度结构（数量与指标映射）由产品/研发统一维护，运营侧不可增删维度 | 维度名称非空且不重复；加权模式下权重合计须为 100% |
