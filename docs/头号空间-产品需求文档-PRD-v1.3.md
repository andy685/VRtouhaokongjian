...
> **版本**: v2.11
> **日期**: 2026年7月10日
> **状态**: 代理商分润公式简化 — 不再扣除供应商成本，直接按「商家充值额 × 分润比例」计算（§2.3/§4.8.6/§10.8）
> **密级**: 内部公开
> **作者**: 产品团队
...
> 📎 v2.8 更新: 新增异常订单处理（§4.6.3）和异常订单审核（§4.8.4.2）两个独立章节 — 商家端6类订单列表页统一支持标记/取消标记异常订单；对账中心支持 pending→escalated→rejected→escalated→resolved 完整状态机流转；平台端支持通过/退款/驳回三种审核操作，驳回原因必填并透传商家端；商家端驳回订单展示驳回原因并支持修正重提或自行解决；收银系统（cashier-ui）经评估不需补齐异常标记功能（角色分工决定异常标记属于管理后台对账环节）
