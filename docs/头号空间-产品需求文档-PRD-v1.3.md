...
| 已选会员且需补差 | `确认并支付 ¥XX.XX` | 微信 / 支付宝 / 现金 |
...
| **支付** | 支持微信/支付宝/现金/会员资产自动抵扣。微信/支付宝使用服务商模式；预存款和游戏币仅在点击结算确认后发生扣减 |
...
| 交互场景 | 交互流程 |
|---------|---------|
| **到店消费（收银开单）** | 顾客到前台 → 收银员选"单次消费"Tab → 可选会员识别 → 点击游戏加入结算单 → 系统试算会员价/优惠/预存款/游戏币/补差金额 → 收银员确认扣款或外部补差 → 支付成功 → 打印小票 → 分配VR设备 |
