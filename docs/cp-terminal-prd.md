...
### 3.1 数据中心（含数据概览、点播消费记录、点播游戏汇总）

> **v1.1 更新**：将原"数据中心"和"数据分析"两个一级菜单合并为一个"数据中心"菜单组，包含数据概览、点播消费记录、点播游戏汇总三个子页面。CP 端只展示游戏级别的数据，不暴露店铺和设备信息。

#### 3.1.1 数据概览 `/cp/dashboard`
...
#### 3.1.2 点播消费记录 `/cp/analytics/consumption`

> ⚠️ **与商家端区别**：CP 端仅展示游戏级别的点播消费数据，不暴露店铺名称和设备信息。

| 模块 | 说明 |
|------|------|
| **筛选条件** | 游戏名称（下拉搜索）+ 日期范围 |
| **表格列** | 游戏名称、消费人数、消费次数、游戏豆消耗、CP收益 |
| **数据来源** | 该 CP 旗下所有游戏在全平台的消费汇总 |

#### 3.1.3 点播游戏汇总 `/cp/analytics/game-summary`

> ⚠️ **与商家端区别**：CP 端按游戏维度汇总，不拆分到店铺。
...
// 参考 router/index.ts 中的模式，CPLayout 作为新布局
{
  ...
  children: [
    ...
    { path: 'analytics/consumption', component: () => import('../views/cp/ConsumptionRecord.vue') },
    { path: 'analytics/game-summary', component: () => import('../views/cp/GameSummary.vue') },
