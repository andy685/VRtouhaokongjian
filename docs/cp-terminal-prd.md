...
```
CP 供应商 ──提交游戏＋定价──▶ 平台超管 ──审核分发──▶ 商家/门店 ──消费──▶ 终端用户
    ▲                           │                      │
    └──── 按次结算 ◀─────────────┴──── 结算对账 ◀────────┘
            （CP定成本价，平台赚差价）        （提现时扣支付渠道手续费）
```
...
- **激励闭环**：游戏被消费 → 按次结算 → 提现（扣除支付渠道手续费），形成正向激励
- **定价权在CP**：CP 自行设定单次启动成本，平台按成本价结算，平台赚取与商家端的游戏豆差价
...
> ⚠️ **v1.2 更新**：平台无分成概念。CP 的收益 = 各游戏体验次数 × 单次成本价（CP 自定）。平台赚取的是商家端游戏豆售价与 CP 成本价之间的差价。提现时由支付渠道（如拉卡拉）扣取提现手续费，与平台无关。
...
| 游戏详情 `GameDetail.vue` | 新增审核操作区（通过/不通过+原因）、查看 CP 信息、查看建议成本价 |
| 内容分发 `ContentDistribute.vue` | 分发时可看到 CP 来源信息 |
| **🆕 CP 管理页** | 新增：CP 列表、审核入驻、合同管理、结算配置 |
